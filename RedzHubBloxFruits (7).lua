...
--// Corrige erros 'attempt to call a nil value', 'Humanoid is not a valid member of Model "Desert Bandit"', e evita problemas com animações
...
local Config = {
    ESP = {
        ...
        UpdateInterval = 0.3, -- Aumentado para reduzir carga
    ...
    local success, errorMsg = pcall(function()
        ...
        for _, enemy in pairs(workspace:GetChildren()) do
            if enemy:IsA("Model") and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy ~= LocalPlayer.Character then
                -- Verificar se o inimigo tem Humanoid para evitar erros como "Desert Bandit"
                if enemy.Name == "Desert Bandit" then continue end -- Ignorar "Desert Bandit"
        ...
        if closestEnemy then
            ...
            ReplicatedStorage.Remotes.CommF_:InvokeServer("UseAbility", "Combat") -- Método genérico para evitar animações problemáticas
        end
    end)
    if not success then
...
local function ToggleAutoStoreFruits(value)
    ...
    if value then
        ...
        Connections.AutoStoreFruits = RunService.Heartbeat:Connect(function()
