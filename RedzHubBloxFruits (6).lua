...
--// Corrige o erro 'attempt to call a nil value' com carregamento robusto da biblioteca Fluent
--// Inclui ESP, Teleport, Auto Farm, Auto Quest, Auto Awakening, Kill Aura, Auto Stats, No-Clip, Fruit Sniping, Server Hop, Event Tracker, Auto Buy, Anti-AFK, Auto Mirage Island, Auto Leviathan Hunt, Auto Race V4, Auto Sea Events, Auto Mastery Farm, Auto Kitsune Shrine e notificações
...
-- Função para carregar bibliotecas com segurança (Corrigido para evitar 'attempt to call a nil value')
local function SafeLoadString(urls, name)
    for _, url in ipairs(urls) do
        ...
        if success and result then
            print("Biblioteca " .. name .. " carregada com sucesso de " .. url)
            return result
        else
            warn("Falha ao carregar " .. name .. " de " .. url .. ": " .. tostring(result))
        end
    end
    return nil
end

-- URLs para tentar carregar a biblioteca Fluent
local FluentURLs = {
    "https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua",
    "https://raw.githubusercontent.com/dawid-scripts/Fluent/master/main.lua",
    "https://pastebin.com/raw/FluentBackupURL" -- URL fictícia como exemplo
}
...
-- Carregar bibliotecas com segurança
local Fluent = SafeLoadString(FluentURLs, "Fluent")
local SaveManager = SafeLoadString(SaveManagerURLs, "SaveManager")
local InterfaceManager = SafeLoadString(InterfaceManagerURLs, "InterfaceManager")

-- Verificar se as bibliotecas foram carregadas
if not Fluent or not SaveManager or not InterfaceManager then
    local errorMsg = "Erro crítico: Não foi possível carregar a biblioteca Fluent. Verifique sua conexão ou executor."
...
local Window = Fluent:CreateWindow({
    ...
    TabWidth = 180, -- Aumentado para melhor visibilidade no mobile
    Size = UDim2.fromOffset(600, 500), -- Tamanho maior para mobile
    Acrylic = true,
    Theme = "Darker",
...
local Config = {
    ESP = {
        ...
        TextSize = 16, -- Aumentado para mobile
        OutlineColor = Color3.fromRGB(0, 0, 0),
        UpdateInterval = 0.5, -- Aumentado para reduzir carga
        MaxRenderDistance = 5000 -- Reduzido para melhorar performance
    },
    KillAuraRange = 20,
    SpeedHackValue = 50,
    DefaultWalkSpeed = 16,
...
local State = {
    ...
    AutoKitsuneShrineEnabled = false
}
...
local function CreateESP(object, type)
    ...
    billboard.Size = UDim2.new(0, 120, 0, 60) -- Aumentado para mobile
...
-- Função para atualizar ESP (Otimizada)
local function UpdateESP()
    ...
    for type, objects in pairs(ESP) do
        for object, esp in pairs(objects) do
            ...
            if distance > Config.ESP.MaxRenderDistance then
...
-- Função para verificar novos objetos (Otimizada com task.defer)
...
-- Função para Auto Quest (Melhorada)
...
-- Função para Auto Kitsune Shrine (Nova)
local function StartAutoKitsuneShrine()
    if not State.AutoKitsuneShrineEnabled then return end
    local success, errorMsg = pcall(function()
        local kitsuneShrine = workspace:FindFirstChild("KitsuneShrine")
        if kitsuneShrine then
            local shrinePos = kitsuneShrine:FindFirstChild("PrimaryPart") and kitsuneShrine.PrimaryPart.Position or Vector3.new(-6500, 10, 7500)
            TeleportToPosition(shrinePos)
            if ReplicatedStorage:FindFirstChild("Remotes") and ReplicatedStorage.Remotes:FindFirstChild("CommF_") then
                ReplicatedStorage.Remotes.CommF_:InvokeServer("KitsuneEvent", "Interact")
            end
        else
            Fluent:Notify({ Title = "RedzHub", Content = "Kitsune Shrine não encontrada!", Duration = 3 })
        end
    end)
    if not success then
        Fluent:Notify({ Title = "RedzHub", Content = "Erro no Auto Kitsune Shrine: " .. tostring(errorMsg), Duration = 3 })
        State.AutoKitsuneShrineEnabled = false
        if Connections.AutoKitsuneShrine then Connections.AutoKitsuneShrine:Disconnect() Connections.AutoKitsuneShrine = nil end
    end
end

-- Função para ativar/desativar Auto Kitsune Shrine
local function ToggleAutoKitsuneShrine(value)
    State.AutoKitsuneShrineEnabled = value
    if value then
        Fluent:Notify({ Title = "RedzHub", Content = "Auto Kitsune Shrine ativado!", Duration = 3 })
        Connections.AutoKitsuneShrine = RunService.Heartbeat:Connect(function()
            local success, errorMsg = pcall(StartAutoKitsuneShrine)
            if not success then
                Fluent:Notify({ Title = "RedzHub", Content = "Erro no Auto Kitsune Shrine: " .. tostring(errorMsg), Duration = 3 })
                State.AutoKitsuneShrineEnabled = false
                if Connections.AutoKitsuneShrine then Connections.AutoKitsuneShrine:Disconnect() Connections.AutoKitsuneShrine = nil end
        ...
        Fluent:Notify({ Title = "RedzHub", Content = "Auto Kitsune Shrine desativado!", Duration = 3 })
        if Connections.AutoKitsuneShrine then Connections.AutoKitsuneShrine:Disconnect() Connections.AutoKitsuneShrine = nil end
    end
end
...
workspace.DescendantAdded:Connect(function(obj)
    ...
    elseif obj.Name == "SeaBeast" then
        Fluent:Notify({ Title = "RedzHub", Content = "Sea Beast spawnado
